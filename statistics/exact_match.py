import json
import csv
from helper_functions.helper_functions import is_match, is_unsure, numeric_ground_truth_match
...
class ModelResults:
    ...
    def exact_match(self):
        ...
        for entry, gt_row in zip(self.data, self.ground_truth):
            ...
            numeric_result = numeric_ground_truth_match(predicted, updated_answer, outdated_answer)

            if is_match(predicted, "Unsure") or is_unsure(predicted):
                wrong_count += 1
            elif is_match(predicted, updated_answer) or numeric_result == "current":
                correct_count += 1
            elif is_match(predicted, outdated_answer) or numeric_result == "outdated":
                outdated_count += 1
            elif numeric_result == "wrong":
                wrong_count += 1
            else:
                unable_exact_match_count += 1
        
        return {
            "correct": correct_count,
            "outdated": outdated_count,
            "wrong": wrong_count,
            "unable_exact_match": unable_exact_match_count,
        }
